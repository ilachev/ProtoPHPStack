...
final readonly class TransportContractGenerator implements CodeGeneratorModule
{
    ...
    public function generateForProtoFile(ProtoFileDescriptor $protoFile, TypeResolver $typeResolver): array
    {
        ...
        foreach ($services as $service) {
            ...
            foreach ($methods as $method) {
                ...
                $endpointBindings[$endpointInterface] = $endpointImplementation;
        ...
        if ($endpointBindings !== []) {
            $files[] = $this->generateEndpointBindingManifest($protoFile, $endpointBindings);
        }
    ...
    /**
     * @param array<class-string, class-string> $endpointBindings
     */
    private function generateEndpointBindingManifest(
        ProtoFileDescriptor $protoFile,
        array $endpointBindings,
    ): GeneratedFile {
        $sourceName = $protoFile->getName();
        if ($sourceName === '') {
            throw new \RuntimeException('Endpoint binding generation requires the source proto file name');
        }
        ...
        return new GeneratedFile(
            rtrim($this->options->getOutputDir(), '/')
            . '/Generated/EndpointBindings/'
            . $normalizedSourceName,
            $this->renderEndpointBindingManifest($endpointBindings),
        );
    }
     ...
     * @param array<class-string, class-string> $endpointBindings
     */
    private function renderEndpointBindingManifest(array $endpointBindings): string
    {
        ksort($endpointBindings);
        $bindingsCode = var_export($endpointBindings, true);
