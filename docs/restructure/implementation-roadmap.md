...
- examples не должны быть обязательной частью запуска шаблона.
...
После физического разделения на `src/Capabilities` и `src/Examples` следующий правильный шаг такой:

1. вычистить legacy-импорты и соглашения, которые всё ещё ожидают `Modules`;
2. отделить auth primitives от demo auth flow;
3. обновить onboarding и current architecture docs под реальную структуру кода;
4. решить судьбу top-level legacy-папок и их допустимого остатка.
